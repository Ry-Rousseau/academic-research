# table_A11.R - Generate Table A11 from supplementary material
# DC/PF Effect on Bailout Policy Index - Corruption Variables with Interactions

# Initialize results storage
model_results_tab_A11 <- model_results_empty

# For Table A11, we need to control for corruption variables and include interactions
...
for (d_type in democracy_types) {
  ...
  for (corr_var in corruption_vars) {
    # Base formula with corruption variable and interaction term
    base_formula <- paste0("index_canon_no ~ polity2 + e_gdppc + FixedER + DC_Mand_Revise2 + ", 
                           corr_var, " + DC_Mand_Revise2:", corr_var)
    ...
    for (i in 1:2) {
      ...
      if (!is.null(model_mi)) {
        # Extract results for DC_Mand_Revise2, corruption variable, and interaction term
        int_term <- paste0("DC_Mand_Revise2:", corr_var)
        for (var_of_interest in c("DC_Mand_Revise2", corr_var, int_term)) {
          ...
          model_results_tab_A11 <- rbind(model_results_tab_A11, data.frame(
...
# Save raw results
write.csv(model_results_tab_A11, file.path(TABLE_PATH, "table_A11.csv"), row.names = FALSE)